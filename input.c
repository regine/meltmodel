...
//RH    char  outcontrolname[61];
  ...
  namedatesmassbal= (char*)calloc(80,sizeof(char)); 
   ...
   printf("\n\n\n\n\n\n ***********INPUTFILE : input.dat *****************\n");
    ...
    fscanf(in,"%f",&daysscreenoutput);         readrestofline(&in);
...
/**** IF DISCHARGE TO BE CALCULATED *****************************/
   ...
   if (disyes >= 1)
    ...
    fscanf(in,"%d",&calcgridyes);     readrestofline(&in);
  ...
  if(ddfyes == 1)    /*ddf can only be caclulated if postemp and melt array are produced*/
   ...
   if(numbersnowdaysout == 0)
    ...
    { for(i=1;i<=numbersnowdaysout;i++)
      ...
      readrestofline(&in);
   fscanf(in,"%d",&winterbalyes);              readrestofline(&in);
   fscanf(in,"%f%f",&winterjdbeg,&winterjdend);    readrestofline(&in);  
   fscanf(in,"%d",&summerbalyes);              readrestofline(&in);
   fscanf(in,"%f%f",&summerjdbeg,&summerjdend);    readrestofline(&in);
...
/*** TIME SERIES OF GLACIER MEAN ***/
    fscanf(in,"%d",&do_out_area);      readrestofline(&in);
  ...
  switch (do_out_area)
   {  case 0:  printf("output of time series area mean   : NO");
	       break;
      case 1:  printf("output of time series area mean   : YES");
   }

/***OUTPUT OF INDIVIDUAL GRID RESULTS FOR EVERY TIME STEP*/

    fscanf(in,"%d",&outgridnumber);    readrestofline(&in); 
...
/****** METHODS RADIATION ************************************************/
    ...
    fscanf(in,"%d",&methodinisnow);  readrestofline(&in);
    ...
    { case 1:  printf("\nALBEDO / SURFACE TYPE READ FROM FILES\n"); 
      ...
      default : printf("\n\n ERROR in input.dat: variable methodinisnow\n\n");
		exit(10);   fclose(outcontrol);
    }

    fscanf(in,"%d",&methodsnowalbedo);     readrestofline(&in);
    fscanf(in,"%d",&methodglobal);         readrestofline(&in);
    fscanf(in,"%d",&methodlonginstation);  readrestofline(&in);
    fscanf(in,"%d",&methodlongin);         readrestofline(&in);
    fscanf(in,"%d",&methodsurftempglac);   readrestofline(&in);
      readrestofline(&in);
    ...
    fscanf(in,"%d",&methodturbul);         readrestofline(&in);    
    if((methodturbul < 1) || (methodturbul > 4))
     { printf("\n\n ERROR in input.dat: methodturbul (=%d) must be 1, 2, 3 or 4 !!! \n\n",methodturbul);
        exit(2);
     }
     
    fscanf(in,"%d",&method_z0Te);         readrestofline(&in);
    fscanf(in,"%d",&methodiceheat);        readrestofline(&in);
    fscanf(in,"%d",&methodnegbal);         readrestofline(&in);

  if(energymethod == 1)
   {
     switch(methodsnowalbedo)
    { case 1:  printf("SNOW ALBEDO CONSTANT\n");
	       fprintf(outcontrol,"SNOW ALBEDO CONSTANT albyes is set to 0\n");
               albyes=0;
	       break;
      case 2:  printf("SNOW ALBEDO GENERATED\n");
	       break;
      case 3:  printf("SNOW ALBEDO GENERATED INCLUDING CLOUD DEPENDENCE\n");
	       break;
      case 4:  printf("SNOW ALBEDO GENERATED AS A FUNCTION OF SNOWDEPTH\n");
	       break;
      case 5:  printf("SNOW ALBEDO GENERATED AS A FUNCTION OF SNOWDEPTH INCLUDING CLOUD DEPENDENCE\n");
	       break;
      case 6:  printf("SNOW ALBEDO ACCORDING TO SICART PHD P.243\n");
	       break;
    ...
    { case 1:  printf("LONGWAVE INCOMING RADIATION FROM MEAS OF NET, GLOB, REF\n"); 
	       break;
      case 2:  printf("LONGWAVE IN FROM DIRECT MEASUREMENTS\n");
	       break;
      case 3:  printf("LONGWAVE INCOMING RADIATION PAR. BY KONZELMANN\n");
	       break;
      case 4:  printf("LONGWAVE INCOMING RADIATION PAR. BY BRUNTS\n");
	       break;
      case 5:  printf("LONGWAVE INCOMING RADIATION PAR. BY BRUTSAERT\n");
	       break;
      case 6:  printf("LONGWAVE INCOMING RADIATION PAR. BY KONZELMANN BUT CLOUDS PARAMETERIZED\n");
	       break;
    ...
    switch(methodlongin)
    { case 1:  printf("LONGWAVE INCOMING RADIATION CONSTANT IN SPACE\n"); 
	       fprintf(outcontrol,"LONGWAVE INCOMING RAD CONSTANT IN SPACE linyes set to 0\n"); 
		linyes = 0;    /*!!!!!!!!!!*/
		  /*SET TO 0, TO AVOID ARRAY STORAGE RESERVATION*/
	       break;
      case 2:  printf("LONGWAVE INCOMING RADIATION VARIABLE IN SPACE\n");
	       break;
    ...
    { case 1:  printf("SURFACE TEMP OF GLACIER IS CONSTANT 0 degrees\n"); 
	       fprintf(outcontrol,"SURFACE TEMP IS CONSTANT  loutyes set to 0\n"); 
	        ...
	        break;
      case 2:  printf("SURFACE TEMP ICE, SNOW LOWERED BY ITERATION IF ENBAL NEG \n");
	        break;
      case 3:  printf(" \nSURFACE TEMP FROM LONGOUT MEASUREMENTS AND (DECREASED WITH ELEVATION) \n");
               printf("     OPTION NOT YET OPERATIONAL\n\n");
	       /*exit(3); jes*/
	        break;
      case 4:  printf("SURFACE TEMP ICE, SNOW CALCULATED WITH SNOW/ICE MODEL\n");
               methodiceheat=1;
	       methodnegbal=1;
    ...
    switch(methodturbul)
    { case 1:  printf("TURBULENCE ACCORDING TO ESCHER-VETTER\n"); 
	       break;
      case 2:  printf("TURBULENCE ACCORDING TO AMBACH - NO STABILITY\n");
	       break;
      case 3:  printf("TURBULENCE ACCORDING TO MUNRO (with stability)\n");
	       break;
      case 4:  printf("CHR TURBULENCE ACCORDING TO MUNRO (with stability)\n"); /*CHR added option*/
	       break;
    ...
    { case 1:  printf("NO ICE HEAT FLUX\n"); 
	       break;
      case 2:  printf("ICE HEAT FLUX CALCULATED\n");
	       break;
    ...
    switch(methodnegbal)
    { case 1:  break;
      case 2:  printf("NEGATIVE ENERGY BALANCES STORED\n");
	       break;
...
/******* SCALING ***********************************/
  readrestofline(&in);
    fscanf(in,"%d",&scalingyes);      readrestofline(&in);
...
/*-------------------------------------------------*/
   ...
   printf(" \t longitude = %6.2f \t latitude = %6.2f \n",laenge,breite);
    ...
    fscanf(in,"%d",&rowclim);     readrestofline(&in);    
    fscanf(in,"%d",&colclim);     readrestofline(&in);
    fscanf(in,"%d %f",&climoutsideyes,&heightclim);     readrestofline(&in);
...
*/
    ...
    fscanf(in,"%f",&gridsize);   readrestofline(&in);    
    fscanf(in,"%f",&timestep);   readrestofline(&in);
    ...
    readrestofline(&in);                  /*READ COMMENT LINE*/
    fscanf(in,"%d",&formatclimdata);      readrestofline(&in); 
  ...
  printf(" number of columns in climate input file= %d\n",maxcol);
    ...
    fscanf(in,"%f",&ERAtempshift);    readrestofline(&in);
    fscanf(in,"%f",&ERAwindshift);    readrestofline(&in);
    ...
    for (i=1; i<=12; i++)
      fscanf(in, "%f", monthtempgrad+i);     readrestofline(&in);

    fscanf(in,"%d",&monthtempscenyes);
    for (i=1; i<=12; i++)
      fscanf(in, "%f", monthtempscen+i);     readrestofline(&in);

    fscanf(in,"%d",&monthprecipscenyes);
    for (i=1; i<=12; i++)
      fscanf(in, "%f", monthprecipscen+i);     readrestofline(&in);
...
/*---- SURFACE TYPE FILES ----*/
    readrestofline(&in);
    fscanf(in,"%d",&n_albfiles);    readrestofline(&in); /*number Alb-files*/
...
*/
    ...
    fscanf(in,"%f",&albsnow);    readrestofline(&in);   /*ALBEDO-VALUES*/
...
/******** RADIATION **********************************************/    
    readrestofline(&in);
    fscanf(in,"%d",&split);      readrestofline(&in);
    
    fscanf(in,"%f",&prozdiffuse);   readrestofline(&in);
    fscanf(in,"%f",&trans);         readrestofline(&in);
    fscanf(in,"%f",&startratio);    readrestofline(&in);
       /*variable ratio is set to startratio at begining of main program*/
    fscanf(in,"%f",&ratiodir2dir);  readrestofline(&in);
    fscanf(in,"%f",&surftemplapserate);  readrestofline(&in);
    ...
    fscanf(in,"%d",&directfromfile);   readrestofline(&in); 
    fscanf(in,"%s",pathdirectfile);    readrestofline(&in);
    ...
    fscanf(in,"%f",&daysdirect);       readrestofline(&in); 
  ...
  if (directfromfile == 1)    /*direct radiation is read from existing files*/       
   ...
   { shayes = 0;   exkyes = 0;
    ...
    fscanf(in,"%d",&slopestation);   readrestofline(&in); 
...
/******* TURBULENCE ********************************/
    readrestofline(&in);
    fscanf(in,"%f",&iterstep);  readrestofline(&in);  /*for iteration of surf temp*/
    fscanf(in,"%f",&z0wice);    readrestofline(&in);  /*roughness length*/
      printf("roughness length for ice = %14.6f\n",z0wice);
    fscanf(in,"%f",&dividerz0T);     readrestofline(&in);  /*divide z0w by this value to get z0T*/
    fscanf(in,"%f",&dividerz0snow);  readrestofline(&in);  /*to get roughness length for snow*/
    fscanf(in,"%f",&z0proz);  readrestofline(&in); 
    fscanf(in,"%f",&z0min);  readrestofline(&in); 
    fscanf(in,"%f",&z0max);  readrestofline(&in); 
...
/**************** PRECIPITATION  ****************/
    ...
    fscanf(in,"%d",&methodprecipinterpol);    readrestofline(&in);
    ...
    fscanf(in,"%f",&precgrad);    readrestofline(&in);
 
        printf("precipitation gradient lower part= %8.2f\n",precgrad);
    fscanf(in,"%f",&precgradhigh);
        printf("precipitation gradient upper part = %8.2f\n",precgradhigh);		
    fscanf(in,"%f",&precgradelev); readrestofline(&in);
    fscanf(in,"%f",&preccorr);    readrestofline(&in);
        printf("precipitation correction = %8.2f\n",preccorr);
    fscanf(in,"%f",&snowmultiplierglacier);    readrestofline(&in);
    fscanf(in,"%f",&snowmultiplierrock);    readrestofline(&in);

    fscanf(in,"%f",&T0);          readrestofline(&in); 
    fscanf(in,"%d",&onlyglacieryes);   readrestofline(&in); 
    fscanf(in,"%f",&glacierpart); readrestofline(&in);
...
/**************** DISCHARGE *********************/
    ...
    fscanf(in,"%s",nameqcalc); readrestofline(&in);
    fscanf(in,"%f",&nodis);        readrestofline(&in);    

/**************** DISCHARGE STORGAE CONSTANTS*********************/

    fscanf(in,"%f",&firnkons);    readrestofline(&in);
    fscanf(in,"%f",&snowkons);    readrestofline(&in);
    fscanf(in,"%f",&icekons);     readrestofline(&in);
    fscanf(in,"%f",&rockkons);     readrestofline(&in);
    ...
    fscanf(in,"%f",&qfirnstart);  readrestofline(&in);
    fscanf(in,"%f",&qsnowstart);  readrestofline(&in);
    fscanf(in,"%f",&qicestart);   readrestofline(&in);
    fscanf(in,"%f",&qrockstart);   readrestofline(&in);
    fscanf(in,"%f",&qground);     readrestofline(&in);  
    fscanf(in,"%f",&jdstartr2diff);readrestofline(&in);
    readrestofline(&in);

    fscanf(in,"%d",&disyesopt);   readrestofline(&in);
    ...
    if (disyesopt == 1)
      printf("Opti=%d  DISCHARGE OPTIMIZATION : YES",disyesopt);     
       ...
       fscanf(in,"%s",optkA);        readrestofline(&in);    
       fscanf(in,"%f",&startopt1);   readrestofline(&in);
       fscanf(in,"%f",&stepopt1);    readrestofline(&in);
       fscanf(in,"%d",&anzahlopt1);  readrestofline(&in);
       fscanf(in,"%s",optkB);        readrestofline(&in);
       
       fscanf(in,"%f",&startopt2);   readrestofline(&in);
       fscanf(in,"%f",&stepopt2);    readrestofline(&in);
       fscanf(in,"%d",&anzahlopt2);  readrestofline(&in);
       fscanf(in,"%s",namematrix);   readrestofline(&in);
...
/*=====================================================================*/
      readrestofline(&in);
    fscanf(in,"%d",&percolationyes);  readrestofline(&in);
    ...
    fscanf(in,"%d",&slushformationyes);  readrestofline(&in);
    ...
    fscanf(in,"%d",&densificationyes);  readrestofline(&in);
        if((densificationyes != 0) && (densificationyes != 1))
          {  printf(" \n variable densificationyes wrong in input.dat =%d\n",densificationyes);
             printf("    must be 0 or 1 (function input.c)\n\n");
             exit(4);
          }
    fscanf(in,"%d",&wetstartyes);  readrestofline(&in);
        if((wetstartyes != 0) && (wetstartyes != 1))
          {  printf(" \n variable wetstartyes wrong in input.dat = %d\n",wetstartyes);
             printf("    must be 0 or 1 (function input.c)\n\n");
             exit(4);
          }
    fscanf(in,"%d",&ndepths);     readrestofline(&in);
    fscanf(in,"%d",&factinter);   readrestofline(&in);
     printf(" \n ndepths=%d   factinter=%d\n",ndepths,factinter);
      ndl = 1;  ndh = (long)ndepths;      /*limit depths  "                    */

       readrestofline(&in);
/*-----------------------*/
    fscanf(in,"%f",&thicknessfirst);  readrestofline(&in);
    ...
    fscanf(in,"%f",&thicknessdeep);  readrestofline(&in);
    ...
    fscanf(in,"%f",&depthdeep);  readrestofline(&in);
    ...
    fscanf(in,"%lf",&denssnow);  readrestofline(&in);
        if(denssnow > 700)
    ...
    fscanf(in,"%d",&irrwatercontyes);  readrestofline(&in);
    ...
    fscanf(in,"%lf",&irrwatercont);  readrestofline(&in);
...
/*---- Output ----------*/
    readrestofline(&in);
    fscanf(in,"%d",&factsubsurfout);  readrestofline(&in);
       if((factsubsurfout < 1) || (factsubsurfout > 24))
          {  printf(" \n variable irrwatercontyes wrong in input.dat = %d\n",factsubsurfout);
             printf("    must be 0, 1 or 2 (function input.c)\n\n");
             exit(4);
          }
    fscanf(in,"%d",&offsetsubsurfout);  readrestofline(&in);

    readrestofline(&in);
    fscanf(in,"%d%d%d%d",&runoffyes,&superyes,&wateryes,&surfwateryes);
    fscanf(in,"%d%d%d",&slushyes,&coldsnowyes,&coldtotyes);  readrestofline(&in);
...
/*if only climate station grid cell computed remove grid output etc that does not make sense*/
    if(calcgridyes == 2)
     {  maxmeltstakes=0;  do_out=0;   surfyes=0;  snowyes=0;  disyes=0;
    ...
    if(methodsurftempglac != 4)     /*snow model is not run*/
     {  runoffyes=0; superyes=0;   wateryes=0;  surfwateryes=0;  slushyes=0;  coldsnowyes=0; coldtotyes=0; precipyes=0; intaccumyes=0;
