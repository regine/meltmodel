...
{  
   ...
   float  x;
     ...
     a0=0.929;  a1=1.13445;  a2=-5.11134;   a3=3.10614;    /*based on all daily data*/
     ...
     if(griddgmdrain[i][j] != nodata)    /*WITHIN AREA TO BE CALCULATED*/
      {
       ...
       switch(inti)
		...
		ALBALT[i][j] = albsnow + snowalbincreaseabs;	  
                      break;
             case 2:  ALBALT[i][j] = albfirn;   /*firn surface, no snow on top*/
                      break;
             case 3:  ALBALT[i][j] = albice;    /*ice surface*/
                      break;
             case 4:  ALBALT[i][j] = albrock;   /*rock surface*/
                      break;
             case 5:  ALBALT[i][j] = albrock;   /*debris surface*/
                      break;
...
void albedocalc()
{
  ...
  switch(inti)   /*** WRITE ENERGY BALANCE GRID-OUTPUT-FILES ***/
   ...
   {  case 1: albedosnownew();       /*snow surface*/
              break;
      case 2: albedosnownew();       /*firn, same procedure as for snow*/
              break;
      case 3:   /*ice linear decrease with elevation*/
              ALBEDO[i][j] = albice-albice*
              ((griddgm[rowclim][colclim]-griddgm[i][j])*albiceproz/100/100);   
              break;
      case 4: ALBEDO[i][j] = albrock;
              break;
      case 5: ALBEDO[i][j] = albrock;
              break;
