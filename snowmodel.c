...
void albedocalcdepth()
{
  ...
  switch(inti)   /*** WRITE ENERGY BALANCE GRID-OUTPUT-FILES ***/
   ...
   {  case 1: albedosnowdepth();       /*snow surface*/
              break;
      case 2: albedosnowdepth();       /*firn, same procedure as for snow*/
              break;
  ...
  { for (jc = 1; jc <= kcols; jc++)
    { i = ir;
     ...
     if ((snowlayer[i][j] == 0.0) && (FIRN[i][j] > 0.))
       ...
       if (snowlayer[i][j] == 0.0) lowerpart = 2;
     sum=0.0;
...
void changegrid()
 {
  ...
  if (percolationyes == 1)
  { testabla=0; 
    dummyabla = ABLA[i][j]-MELT[i][j];}  /*note NO factor 1000 necessary because density is in g/kg*/
  if (percolationyes == 0)
