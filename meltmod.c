...
/******* INTERPOLATION OF AIR TEMPERATURE ******/
 ...
 if(methodglobal == 1)    /*if methodglobal=2, topofatm already calculated; cloud parameterization with G/Toa ratio*/
  ...
  switch(methodlonginstation)    /*HOW IS LONGWAVE INCOMING AT STATION DETERMINED*/
   { case 1:  longinstationnetradmeas();     /*FROM MEAS NET, GLOB, REF*/
              break;
     case 2:  break;                   /*LONGIN READ FROM CLIMATE DATA INPUT FILE*/
                /*has been read into LWin in readclim*/
     case 3:  longinstationkonzel();   /*PARAMETERIZATION Konzelmann et al. using cloud amount*/
	      break;
     case 4:  longinstationbrunts();
	      break;
     case 5:  longinstationbrutsaert();
	      break;
     case 6:  longinstationkonzel();   /*as 3, but clouds are parameterized, function called there*/
	      break;
...
/*LONGWAVE OUT RADIATION AT CLIMATE STATION FROM MEAS*/
  if(methodsurftempglac == 3)   /*use longwave outgoing measurements at climate station*/   
   {  i=rowclim;  j=colclim;   /*needed because surtempfromlongout also used later for entire grid*/
...
/*IF NO SURFACE TEMP ITERATION THERE IS NO COMPUTATION OF STABILITY FUNCTIONS FOR
  ...
  FOR CLIMATE STATION */
   if((methodsurftempglac != 2) && (methodsurftempglac != 4))
    {  i=rowclim;  j=colclim;    /*ONLY AT CLIMATE STATION GRID CELL*/
...
/*================================================================================*/
 ...
 if(calcgridyes == 2)      /*computation only for climate station grid cell*/
  {  firstrow = rowclim;  lastrow = rowclim;
     firstcol[firstrow] = colclim;  lastcol[firstrow] = colclim;
 ...
 /********* ALBEDO **********************/ 
	...
	{ case 1: albedocalcconst();    /*constant albedo for snow/slush/ice*/
	  ...
	  case 3: albedocalc();  /*as 2 but incl. cloud dependence*/
   		  break;
	  case 4: albedocalcdepth();  /*as 2 but depending on snowdepth*/
   		  break;	
 ...
 /* at start in initial.c set to 0 for area calculated*/
   switch(methodsurftempglac)
	...
	case 2:  /*ITERATION SO THAT ENBAL IS BALANCED, START WITH TEMP=0, THEN LOWER TEMP*/ 
   		surftemp[i][j] = 0;
   		break;
	case 3: if(surftemplapserate == 0)	  /* measured surf temp constant in space*/
	          surftemp[i][j] = surftemp[rowclim][colclim]; 
...
do          /*goes only once through the loop in case methodsurftempglac is not 2*/
 {      
  ...
  switch(methodturbul)
   {  case 1: 	sensescher();    /*ACCORDING TO ESCHER-VETTER*/
      		latescher();
		break;
      case 2: 	sensible();   /*NO STABILITY, STAB FUNCTIONS = 0*/
      		latent();
		break;
      case 3: 	sensible();   /*INCLUDING STABILITY, SAME FUNCTIONS AS 2 BUT*/
      		latent();     /*STAB FUNCTIONS HAVE BEEN DETERMINED BEFORE */
		break;        /*STAB FUNCTIONS SPATIALLY NOT VARIABLE*/
      case 4: 	sensiblenew();   /*SAME AS 3, BUT DIFFERENT WAYS OF COMPUTATION*/
