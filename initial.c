...
/****************** CLIMATE *********************************/
 ...
 /******** DIGITAL TERRAIN MODEL **********************************/
  ...
  ncols  = (int)x[1];     nrows  = (int)x[2];      /*number col, rows      */
  xll    = x[3];     yll    = x[4];      /*left lower corner     */
  cs     = x[5];     nodata = x[6];      /*cellsize, missing data*/

  nrl = 1;  nrh = (long)nrows;      /*limit rows von 1 bis max number rows*/
  ncl = 1;  nch = (long)ncols;      /*limit columns  "                    */
...
/*   snowscenario is added to initial snow cover, defined in variab.h (hidden option) */
 ...
 /*Gelesen wird bis einschl. der Zeile vor dem eigentlichen Startzeitpunkt*/  
   ...
   readrestofline(&inklima);      readrestofline(&inklima);   /*first 2 rows*/
...
/*GRIDOUT FOR EACH GRID*/
 switch(do_out)   /* INITIALISATION FOR ARRAYS WITH DAILY OR PERIOD MEANS */
   {  case 0:  break;
      case 1:  break;               /*Outputfile for every timestep*/
      case 2:  meandayreserve();    /*Outputfile with daily means*/
	       meandaynull();       /*set to zero*/
	       break;
      case 3:  meanallnull();       /*Outputfile with means for whole period*/
	       break;
      case 4:  meandayreserve();    /*daily means and means for whole period*/
	       meandaynull();       
	       meanallnull();
   } /*switch*/
