  ...
  for (i=firstrow;i<=lastrow;i++)   /*only rows and col to be calculated*/       
   ...
   {    
    ...
    { switch(climoutsideyes)   /*climate data not in area to be calculated*/
      { case 0 : tempint[i][j] = temp + (griddgm[i][j]-griddgm[rowclim][colclim])/100 * lapserate;
      		 break;
        case 1 : tempint[i][j] = temp + (griddgm[i][j]-heightclim)/100 * lapserate;
      		 break;
        case 2 : tempint[i][j] = temp + (griddgm[i][j]-heightclim)/100 * lapserate;
...
/*T from files has already been adjusted for bias between ERA and glacier and 
 ...
 { case 1: tempgradient();   break;   /*constant or monthly variable lapse rate from input.dat*/
   case 2: tempgradient();   break;   /*each time step variable lapse rates from climate file*/
...
{  
   ...
   PhiM=0;  PhiH=0;      /*first run with neutral stability*/
...
void readprecipindexmap()   
  ...
  /*** OPEN INDEX  FILE ****/
    ...
    if ((inprecipindexgrid = fopen (dummy,"rt")) == NULL)  {
	   ...
	   exit(4);     fclose(inprecipindexgrid);
    }
...
void precipinterpol()
 ...
 if(prec > 0)      /*precipitation falls in timestep considered*/
 {
   ...
   {  case 0 :  referenceelevation = griddgm[rowclim][colclim];    /*climate station elevation*/
      		break;
      case 1 :  referenceelevation = heightclim;  /*fixed elevation from which extrapolation*/
      		break;
      case 2 :  referenceelevation = griddgm[rowclim][colclim];
...
/*  printf("  precip %5.2f RAIN %5.2f DIR %5.1f\n",LWin,LONGIN[54][102],DIRECT[54][102]);
 ...
 /********* ALBEDO **********************/ 
	...
	{ case 1: albedocalcconst();    /*constant albedo for snow/slush/ice*/
   		  break;
	  case 2: albedocalc();  /*albedo generated as function of T, snow fall*/
   		  break;
	  case 3: albedocalc();  /*as 2 but incl. cloud dependence*/
   		  break;
	  case 4: albedocalcdepth();  /*similar to 2 but depending on snowdepth*/
   		  break;	
	  case 5: albedocalcdepth();  /*as 4 but incl. cloud dependence*/
   		  break;
   	  case 6: albedosnowpoly();  /*modified version of oerlemans and knap, sicart PhD. p.243*/
   		  break;   		  	
        }  /*end case*/
      }  /*endif*/	
  ...
  /************* TURBULENT FLUXES *********************/
   ...
   switch(methodturbul) 
    { case 1: sensescher();   
              latescher();  
              break;
      case 2: sensible();     /*no stability correction, Phi=0*/
              latent();
	      break;
      case 3: sensiblestabilityiteration();   
              /*sensible() called from there after computing stability functions*/
     	      latent();
     	      break;
      case 4: turbfluxes();  /*as 3 but different way: Carleen Tijm-Reijmer, 2/2005*/
     	      break;
    }
