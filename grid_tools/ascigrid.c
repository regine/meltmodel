...
void statistics(float *anzahl,float *mini,float *maxi,float *summe,float *mittel,float *std)
{
  ...
  *maxi=(float)statmaxi;
  *summe=(float)sx;
  if (*anzahl>0) *mittel=(float)( sx / *anzahl); else *mittel=-9999.0;
  if (*anzahl>2) *std   =(float)( sqrt((sx2-sx*sx / *anzahl)/(*anzahl-1))); else *std=-9999.0;
}
...
int main(int anz,char *paramstring[])
{
  ...
  if (anz<2)
  {
    ...
    printf("\n");
    printf("Eingabedatei : "); dateinameein=gets(dateinameein);
